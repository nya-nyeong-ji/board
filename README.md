...
1. 클레스 네임 수정<br>
2. 정렬 기능과 검색 기능의 선택 추가<br><br>


# 2021/06/14<br>
지난 시간 언급되었던 개선 사항들은 수정되었습니다.<br>
클래스 네임을 수정했고 정렬, 검색 기능을 함께 사용할 수 있도록 했습니다.<br>
간단한 게시판의 형태는 갖추게 되었습니다.<br>
하지만 배포하기에는 기능이 모자라다고 여겨집니다.<br>
배포를 하게 될 때 필요하게 될 요구사항들을 정리한 뒤 추가할 기능 요구사항들을 추려낸 뒤 찾아뵙겠습니다.


# 2021/06/21<br>
이때까지 언급되었던 기술들을 바탕으로 Jpa를 이용한 BD 이용법(등록, 수정, 삭제, 조회, 기타 조건절 등)과 특수한 목적을 가진 어노테이션을 간단하게 살펴 볼 수 있었습니다.<br>
이는 앞서 설명했던 기술들로 간단한 게시판은 제작함에 있어 DB에 관련된 문제를 해결할 수 있게 되었다는 것을 의미합니다.<br>
기본 전제였던 '간단한 게시한 만들기'에는 거의 부합한 수준이라고 생각됩니다.<br>
반대로 언급되었던 'Spring security'에 대한 활용이 적었습니다.<br>
간단한 로그인 기능과 관리자 페이지 및 관리자의 게시판 관리 기능 등 권한이나 인증에 관련된 개발 몇 가지를 공부한 다음 배포해보는 것을 목표로 하겠습니다.<br>

1. DB(JPA)관련 기술에 대한 공부는 현 단계에서 충분하다고 판단<br>
2. 보안(Spring Security)에 대한 활용이 부족했다고 판단<br>
3. 로그인을 이용한 회원, 관리자 페이지의 추가, 이 과정에서 권한, 인증에 관련된 기술 공부<br>
4. 이 과정 이후 배포 예정(변경 가능)



# 2021/06/24<br>
회원 관리 과정을 추가하여 사용자 권한과 관련된 과정을 연습해 보겠습니다<br>

## 1. entity
멤버에 권한을 관리하기 위한 칼럼을 추가하였습니다.<br>

## 2. service
회원가입 과정에서 회원으로 가입할 것인지, 관리자로 가입할 것인지 정하는 부분을 추가하였습니다.<br>
그 부분에 맞춰 권한을 다르게 주어주는 과정을 추가하였습니다.<br>

### UploadUerByUsername<br>
로그인 이후 권한을 부여하는 부분입니다.<br>
entity에 추가되었던 권한 칼럼을 이용하여 사용자에게 맞는 권한을 부여합니다.<br>

### CheckAuth<br>
service단계에서 권한을 확인하는 과정을 구현해 보았습니다.<br>
security holder를 이용하여 구현할 수 있었습니다.<br>
결과적으로는 이후 언급될 타임리프를 이용하여 페이지 단계에서 처리가 가능했기 때문에 이를 사용하지 않게 되었습니다.<br>

## 3. html<br>
관리자 페이지(admin.html)가 추가되었으며 타임리프를 사용하여 조건절이나 사용자 권한등을 사용하여 특수한 상황에 대처하였습니다.<br>

### th:if="조건"<br>
조건에 부합하면 해당 부분을 출력합니다.<br>
java, c++의 if문에서 사용하는 ||, && 대신 and, or를 사용합니다.<br>

### authorization.expression(표현식)<br>
권한과 관련할 수 있습니다.<br>
spring security에서 사용되었던 hasRole()등을 사용하여 boolean값으로 사용할 수 있습니다.<br>

### th:disabled="조건"<br>
조건에 부합할 경우 해당 부분을 사용할 수 없도록합니다.

### authentication<br>
spring security의 user detail을 생각하시면 됩니다.<br>
사용자가 가지고 있는 user객체의 username, password, authorities를 사용할 수 있습니다.<br>

spring security, thymeleaf 를 주로 사용하는 과정이었습니다.<br>
thymeleaf 만 잘 사용하여도 페이지 구성을 훨씬 다양하게 할 수 있다는 것을 알 수 있었습니다.<br>

이번 과정으로 권한과 관련된 부분들을 공부할 수 있었습니다.<br>
배포 이전에 관리자를 추가하는 과정이 필요할지 등을 좀 더 생각해 봐야겠습니다.<br>

당장에 페이지에서 조건문을 사용할 수 있던것 만으로 표현할 수 있는 부분이 확연히 증가하였습니다.<br>
그렇기 때문에 배포 이전에 thymeleaf를 좀 더 다뤄보는 것이 좋을 것 같습니다.<br>


# 2021/07/27<br>
한 달만의 수정입니다.<br>
최근 있었던 몇가지 코딩테스트에도 참여를 해봤고 처음으로 Spring boot를 이용한 코딩테스트 역시 거쳐 보았습니다.<br>
개인적으로 느낀 점과 얻은 것이 많은 경험이었습니다.<br>
또한 그 느낀 점들 중 하나가 RestAPI를 배워야 겠다는 점입니다.<br>
위의 예시는 대표적인 것이며, 좀 더 자세히 설명하겠습니다.<br>
현제 이 프로젝트의 가장 큰 문제는 여러 기술들을 적용시키기 위한 노력은 했으나 클린 코드를 비롯 과정들이 적용되지 않았습니다.<br>
코딩테스트를 통해 새롭게 배워야할 기술들을 이 프로젝트에 적용시키면서 다시 한번 코드들을 되살펴 보니 복습조차 어려울 지경입니다.<br>
그렇기 때문에 다음과 같은 결정을 하게 되었습니다<br>
### 적용시킬 기술별로 프로젝트를 따로 생성한다.<br>
8월달부터 코딩테스트로 이래저래 바빠질 예정입니다.<br>
새로운 기술, 복습 등을 소홀히 할 수 없기에 이 프로젝트에서 사용되었던 기술<br>
앞으로 사용하게 될 기술<br>
각 각 기술을 합하여 사용한 프로젝트<br>
그리고 각 프로젝트들을 생성하면서 클린 코드같은 최적화와 본인의 코드에 대한 이해를 최우선적으로 해야함을 실감했습니다.<br>
배포하는 것 까지 목표로 삼았습니다만, 이 프로젝트는 여기에 사용되었던 기술들을 다시 완전히 익혔다고 느껴질 때 즈음 다시 수정할 것 같습니다.<br>
또한 그 때에는 본격적으로 git을 포트폴리오로 사용하기 위해 git의 사용방법 및 작성 노하우 역시 배워올 것 같습니다.